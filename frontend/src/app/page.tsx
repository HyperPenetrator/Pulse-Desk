  ...
  return (
    <div className="min-h-screen bg-surface text-text-primary flex flex-col font-sans selection:bg-brand-primary/20">
      ...
      <header className="relative w-full max-w-6xl mx-auto px-6 py-6 flex justify-between items-center z-10">
        ...
        <div className="text-xs bg-slate-900 border border-slate-800 rounded-full px-3 py-1 text-slate-400">
        ...
        <section className={`bg-glass-bg backdrop-blur-md border border-glass-border rounded-3xl p-6 md:p-8 shadow-glass-dark z-20 transition duration-700 ${
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="symptom-input" className="block text-xs font-semibold uppercase tracking-wider text-slate-400">
                1. Describe Symptoms / Illness
              </label>
              <textarea
                id="symptom-input"
                rows={4}
                className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className="text-xs bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-lg px-2.5 py-1 text-slate-400 hover:text-white transition"
                >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-950 p-1 border border-slate-800 rounded-xl">
                <button
                  type="button"
                  className={`py-2 text-xs font-medium rounded-lg transition ${locationMethod === 'gps' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}
                ...
                <button
                  type="button"
                  className={`py-2 text-xs font-medium rounded-lg transition ${locationMethod === 'manual' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}
              ...
              {locationMethod === 'gps' && (
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-medium text-slate-400">GPS Coordinates Status</div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        type="number"
                        step="any"
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none"
                        value={latitude || ''}
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className={`w-full py-4 rounded-2xl font-bold text-sm tracking-wide uppercase transition duration-200 ${
                loading
          ...
          <section className="absolute inset-0 bg-glass-bg backdrop-blur-md border border-glass-border shadow-glass-dark rounded-3xl p-6 flex flex-col justify-between">
            ...
            {loading && (
              <div id="loading-state">
                ...
                <h3 className="text-sm font-medium text-slate-300">Analyzing triage severity...</h3>
              ...
              <div id="result-container" className="w-full text-left space-y-6">
                <div className="text-center pb-2 border-b border-slate-800">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${
                    result.severity === 'emergency' ? 'bg-rose-950 text-rose-300 border border-rose-900/50' : 'bg-blue-950 text-blue-300 border border-blue-900/50'
                  }`}>
                  ...
                  <div id="dispatch-success-card" className="space-y-4">
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                      <div>
                        ...
                        <div id="facility-name-display" className="text-base font-bold text-white mt-0.5">{result.facility_name}</div>
                      </div>
                ...
                {result.status === 'escalated' && (
                  <div id="escalated-card" className="space-y-4">
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                      <h4 className="text-sm font-bold text-white">Escalated to District-Admin</h4>
                ...
                {result.status === 'triage' && (
                  <div id="triage-non-emergency-card" className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-3">
                    <h4 className="text-sm font-bold text-white">General Clinic Advice</h4>
